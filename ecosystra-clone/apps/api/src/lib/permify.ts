/**
 * Permify HTTP client (Check API + Data write). Set PERMIFY_BASE_URL when running Permify.
 * Schema: see /permify/schema.perm
 *
 * When Permify is unreachable, all operations degrade gracefully:
 *  - check → allow (with warning)
 *  - write/delete → skip (with warning)
 */
...
let _permifyReachable: boolean | null = null;
let _lastWarnedAt = 0;

function warnUnreachable(context: string, err: unknown) {
  const now = Date.now();
  if (now - _lastWarnedAt < 30_000) return; // throttle warnings to once per 30s
  _lastWarnedAt = now;
  _permifyReachable = false;
  console.warn(
...
export async function permifyCheck(input: CheckInput): Promise<boolean> {
  ...
  try {
    ...
    if (isConnectionError(err)) {
      ...
      return true; // allow when Permify is down
    }
    throw err;
  }
}
...
export async function writeTuples(tuples: Tuple[]): Promise<void> {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error("[permify] write failed", res.status, txt);
    ...
    if (isConnectionError(err)) {
      ...
      return; // skip write when Permify is down
    }
    throw err;
  }
}
...
export async function deleteTuple(tuple: Tuple): Promise<void> {
  ...
  try {
    ...
    if (isConnectionError(err)) {
      ...
      return; // skip delete when Permify is down
    }
    throw err;
  }
}

/** Owner + admin on project for assign / task.admin checks. */
export async function ensureProjectTuples(
  boardId: string,
  ownerUserId: string
): Promise<void> {
  await writeTuples([
...
/** Bootstrap tuples for a new board (project) and task (item). */
...
export async function setTaskAssigneeTuple(params: {
  ...
  if (params.previousAssigneeUserId && params.previousAssigneeUserId !== params.assigneeUserId) {
    await deleteTuple({
      ...
      relation: "assignee",
      subjectType: "user",
      subjectId: params.previousAssigneeUserId,
    });
  }
    ...
    await writeTuples([
      {
        ...
        relation: "assignee",
        subjectType: "user",
        subjectId: params.assigneeUserId,
      },
    ]);
  }
}
...
export async function setTaskOwnerTuple(params: {
  ...
  if (params.previousOwnerUserId && params.previousOwnerUserId !== params.ownerUserId) {
    await deleteTuple({
      ...
      relation: "owner",
      subjectType: "user",
      subjectId: params.previousOwnerUserId,
    });
  }
  await writeTuples([
    {
...
export async function addProjectAdmin(params: {
  boardId: string;
  adminUserId: string;
}): Promise<void> {
  await writeTuples([
